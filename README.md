...
- SimConstraints simulation.tip_track_phase: If true, the program will track the approximate location of the spiral wave tip on the estimated value phase angle of the wave about the calculated phase singularity point.
...
This code is designed to be run on the Georgia Tech PACE Phoenix cluster, but can be run on any computer with a NVIDIA GPU and CUDA installed. To run locally, either open the constructed `fhnModel.exe` file or use your terminal to run the program by navigating to the `fhnModel.exe` and run either the `start fhnModel.exe` command (Windows) or the `wine fhnModel.exe` command (Linux and macOS).
